<template>
  <section class="series-detail">
    <div class="series-detail__hero">
      <div class="series-detail__hero-backdrop">
        <img
            :src="heroBackdrop"
            :alt="series.name || 'Serien Backdrop'"
            class="series-detail__backdrop"
        />
        <div class="series-detail__backdrop-overlay"></div>
      </div>

      <div class="series-detail__hero-card">
        <div class="series-detail__poster-wrap">
          <img
              :src="posterImage"
              :alt="series.name || 'Serien Poster'"
              class="series-detail__poster"
          />
        </div>

        <div class="series-detail__hero-copy">
          <p class="series-detail__eyebrow">Series Detail</p>

          <h1 class="series-detail__title">
            {{ series.name || 'Unbekannte Serie' }}
          </h1>

          <p v-if="series.tagline" class="series-detail__tagline">
            {{ series.tagline }}
          </p>

          <div class="series-detail__chips">
            <span class="detail-chip">{{ firstAirYear }}</span>
            <span class="detail-chip">★ {{ ratingText }}</span>
            <span class="detail-chip">{{ seasonsText }}</span>
            <span class="detail-chip">{{ languageText }}</span>
          </div>

          <p class="series-detail__overview">
            {{ series.overview || 'Keine Beschreibung verfügbar.' }}
          </p>
          ...
          <div class="detail-showcase__media">
            <div class="section-head section-head--integrated">
              <div>
                ...
                <h2>Trailer & Seriendetails</h2>
                <p class="section-subtitle section-subtitle--overlay">
                  Der Trailer steht im Fokus, die wichtigsten Serieninformationen folgen dezent darunter.
                </p>
              </div>
            ...
            <div v-else class="empty-box empty-box--feature">
              <p>Für diese Serie ist aktuell kein Trailer verfügbar.</p>
            </div>
          </div>
          ...
          <div class="detail-showcase__body">
            <div class="info-inline">
              <article class="info-inline__item">
                <span class="info-inline__label">Ersterscheinung</span>
                <strong class="info-inline__value">{{ series.first_air_date || 'Unbekannt' }}</strong>
              </article>

              <article class="info-inline__item">
                <span class="info-inline__label">Staffeln</span>
                <strong class="info-inline__value">{{ series.number_of_seasons ?? 'N/A' }}</strong>
              </article>

              <article class="info-inline__item">
                <span class="info-inline__label">Episoden</span>
                <strong class="info-inline__value">{{ series.number_of_episodes ?? 'N/A' }}</strong>
              </article>

              <article class="info-inline__item">
                <span class="info-inline__label">Bewertung</span>
                <strong class="info-inline__value">{{ ratingText }} · {{ voteCountText }} Stimmen</strong>
              </article>
            </div>

            <div
                v-if="series.production_companies?.length"
                class="companies-block companies-block--integrated"
            >
              <div class="subsection-head subsection-head--soft">
                <h3>Beteiligte Firmen</h3>
                <span>{{ series.production_companies.length }}</span>
              </div>

              <div class="companies-grid">
                <article
                    v-for="company in series.production_companies"
                    :key="company.id"
                    class="company-card"
                >
            ...
            <div v-else class="empty-box empty-box--compact">
              <p>Für diese Serie sind keine Produktionsfirmen hinterlegt.</p>
            </div>
          </div>
        </div>
      </section>
...
export default {
  ...
  computed: {
    ...
    posterImage() {
      return this.series.poster_path
          ? `https://image.tmdb.org/t/p/w500${this.series.poster_path}`
          : 'https://via.placeholder.com/500x750?text=No+Poster'
    },
    firstAirYear() {
      return this.series.first_air_date ? this.series.first_air_date.slice(0, 4) : 'Unbekannt'
    },
    ratingText() {
      const rating = Number(this.series.vote_average || 0)
      return `${rating.toFixed(1)} / 10`
    },
    voteCountText() {
      return this.series.vote_count
          ? this.series.vote_count.toLocaleString('de-DE')
          : '0'
    },
    seasonsText() {
      const count = this.series.number_of_seasons
      return count ? `${count} Staffel${count > 1 ? 'n' : ''}` : 'Staffeln offen'
    },
    languageText() {
      return this.series.original_language
          ? this.series.original_language.toUpperCase()
          : 'N/A'
    },
    trailerThumbnail() {
  ...
  methods: {
    async loadSeriesData() {
      ...
      try {
        ...
        console.error('Fehler beim Laden der Serieninformationen:', error)
      }
    },
    ...
    async fetchTrailer() {
      ...
      try {
        const response = await fetch(
            `https://api.themoviedb.org/3/tv/${seriesId}/videos?api_key=${apiKey}&language=de-DE`
        )
...
.series-detail__hero-backdrop {
  position: absolute;
  inset: 0;
}

.series-detail__backdrop {
  width: 100%;
  height: 100%;
  object-fit: cover;
...
.series-detail__backdrop-overlay {
  position: absolute;
  inset: 0;
  background:
